...
it('should be able to request for a password recovery sending notification to the user.', function () {
    ...
    Livewire::test(Recovery::class)
        ->assertDontSee('You will receive an email the password recovery link.')
        ->set('email', $user->email)
        ->call('startPasswordRecovery')
        ->assertSee('You will receive an email the password recovery link.');
...
it('testing email property', function ($value, $rule) {
    Livewire::test(Recovery::class)
        ->set('email', $value)
        ->call('startPasswordRecovery')
